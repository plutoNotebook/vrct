...
@click.option('--double',        help='How often to reduce dt', metavar='TICKS',                    type=click.IntRange(min=1), default=150, show_default=True)
...
# ㅖarameters for Variance Reduced Consistency Training
@click.option('--vrct',          help='Activate vrct',                                              is_flag=True)
@click.option('--schedule',      help='Type of scheduling (perturbed, const).',                     type=click.Choice(['perturbed', 'const']), default='perturbed', show_default=True)
@click.option('-d',              help='How often to reduce schedule.',                              type=int, default=100, show_default=True)
@click.option('--initial',       help='Initial value for perturbed scheduling.',                    type=float, default=0.7, show_default=True)
@click.option('--gamma',         help='Growth rate for perturbed scheduling.',                      type=float, default=0.5, show_default=True)
@click.option('--stf',           help='Stable target field [default: False].',                      is_flag=True)
@click.option('--conv',          help='Freeze conv layer.',                                         is_flag=True)
@click.option('--qkv',           help='Freeze qkv layer.',                                          is_flag=True)
@click.option('--skip',          help='Freeze skip connection.',                                    is_flag=True)
...
def main(**kwargs):
    ...
    if opts.vrct:
        c.loss_kwargs = dnnlib.EasyDict(P_mean=opts.mean, P_std=opts.std, q=opts.q, c=opts.c, k=opts.k, b=opts.b, adj=opts.mapping, gamma=opts.gamma, initial=opts.initial, mu_type=opts.schedule)
    else:
    ...
    # partial freeze
    if opts.vrct:
        c.network_kwargs.update(freeze_conv_layers=opts.conv, freeze_qkv_projections=opts.qkv, freeze_skip_layers=opts.skip)
        c.update(double_ticks_mu=opts.d)
        c.update(stf=opts.stf)

    # Preconditioning & loss function.
    if opts.precond == 'ect':
        if opts.vrct:
            c.network_kwargs.class_name = 'training.networks.VRCTPrecond'
            c.loss_kwargs.class_name = 'training.loss.VRCTLoss'
        else:
